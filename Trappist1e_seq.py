from Trappist1e_script_seq import *
    ...
    if Profil == True :
        ...
        if Layers == False :
            data_convert,h_top = Boxes(data_path,delta_z,Rp,h,P_h,t_selec,g0,M,number,T_comp,P_comp,Q_comp,n_species,X_species,\
                M_species,c_species,m_species,ratio_HeH2,Upper,TopPressure,Surf,Tracer,Cloudy,Middle,LogInterp,TimeSelec)
        else :
...
if Cylindric_transfert_3D == True :
    ...
    if Continuum == True :
        if Kcorr == True :
            k_cont_rmd = np.load("%s%s/%s/k_cont_all_%i%i_%s_%i_%i%i_%i_rmd_%.2f_%.2f_%s.npy"\
            %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,phi_rot,phi_obli,domain))
        else :
            k_cont_rmd = np.load("%s%s/%s/k_cont_all_%i%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
            %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,phi_rot,phi_obli,domain))
    else :
        k_cont_rmd = np.array([])
